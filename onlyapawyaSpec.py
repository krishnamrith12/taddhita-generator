...
#    def checkTrue(self, SabxarUpa):
    ...
    def resolve(self, SabxarUpa, candiList):
        ...
        allSpec = 0
        filterList = {'semantic':[],'morphological':[],'phonological':[],'phonetic':[],'gowra':[],'apawyam':[]}
        for item in candiList:
            try:
                if len(item.specificity) <1:
                    allSpec = -1
            except:
                allSpec = -1
            print 'allSpec is ', allSpec
            if allSpec != -1:
                print candiList
                for item in candiList:
                    print item.specificity,item
                    if 'semantic' in item.specificity:
                        filterList['semantic'].append(item)
                if len(filterList['semantic']) > 1:
                    for stuff in filterList['semantic']:
                        try:
                            if stuff.sense == 'gowra':
                                filterList['gowra'].append(stuff)
                            if stuff.sense == 'apawyam':
                                filterList['apawyam'].append(stuff)
                        except:
                            pass
                        if 'morphological' in stuff.specificity:
                            filterList['morphological'].append(stuff)

                    if len(filterList['gowra']) == 1:
                        candiList = filterList['gowra']
                        break
                    if len(filterList['morphological']) == 1:
                        candiList = filterList['morphological']
                        break
...
class IVone85c(IVone83c):
    ...
    def execute(self,SabxarUpa,replText = 'apawyam'):
        ...
        for item in SabxarUpa.properties['words']:
            if 'semanticSense' in item.properties.keys():
                SabxarUpa.properties['semanticSense'] = item.objs2text()
                replText = SabxarUpa.properties['semanticSense']
    ...
    def execute(self,SabxarUpa,replText='apawyam'):
        for item in SabxarUpa.properties['words']:
            if 'semanticSense' in item.properties.keys():
                ...
                SabxarUpa.replace(SabxarUpa.properties['semanticSense'],aN)
                SabxarUpa.properties['waxXiwaprawyaya'] = aN
                SabxarUpa.properties['prawyaya'] = SabxarUpa.properties['waxXiwaprawyaya']
                for i in range(len(SabxarUpa.properties['words'])):
                    if replText in SabxarUpa.properties['words'][i].objs2text():
...
IVone83.register(IVone84)
IVone83.register(IVone85)
IVone83.register(IVone86)
...
SabxarUpa = SabxaCollection('kapi fas! gowra')
